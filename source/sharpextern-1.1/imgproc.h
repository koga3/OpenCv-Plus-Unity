...
//UFIX
// Неведомая херня происходит на Андройде под Моно и архитектурой arm7
// при вызове оригинальной ф-ии imgproc_drawContours_vector из-за бага, который зависит от 
// последовательности аргументов ф-ии в color приходит херня, больше похожее на напрвильный указатель в памяти
// Минимальная функция на которой воспроизводится баг 
//  (cv::_InputOutputArray *image, cv::Point **contours, int contoursSize1, int *contoursSize2, int contourIdx, MyCvScalar color)
// Если поменять порядок передачи агрументов баг  исчезает, как раз для этого нужна функция imgproc_drawContours_vector_fix
CVAPI(void) imgproc_drawContours_vector_fix(cv::_InputOutputArray *image,
	cv::Point **contours, int contoursSize1, int *contoursSize2,
	MyCvScalar color, int contourIdx, int thickness, int lineType,
	cv::Vec4i *hierarchy, int hiearchyLength, int maxLevel, MyCvPoint offset)
{
	imgproc_drawContours_vector(image, contours, contoursSize1, contoursSize2, contourIdx, color, thickness, lineType, hierarchy, hiearchyLength, maxLevel, offset);
}
