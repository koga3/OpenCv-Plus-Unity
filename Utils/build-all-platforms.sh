...
if [ ${targets[0]} -eq 1 ]; then
	...
	# x64
	required bash ./scripts/build-plugin.sh --type "$type" --platform windows --arch x86_64 --makesys \"Visual Studio 16 2019\" --version $version_plugin $rebuild_option $wrapper_only
fi
